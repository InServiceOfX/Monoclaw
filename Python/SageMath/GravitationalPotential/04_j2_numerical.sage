...
def main():
    ...
    for th in theta_samples:
        ...
        print(f"{float(th*180/pi):<12.1f} {float(cth):<12.4f} {float(p2):<12.4f} {v_j2:<20.6e}")

    # Find max and min of V_J2
    v_vals_only = [v for (_, _, v) in v_j2_values]
    max_idx = v_vals_only.index(max(v_vals_only))
    min_idx = v_vals_only.index(min(v_vals_only))

    th_max, _, v_max = v_j2_values[max_idx]
    th_min, _, v_min = v_j2_values[min_idx]

    print(f"\nMaximum V_J2 at theta = {float(th_max*180/pi):.1f} deg: {v_max:.6e} m^2/s^2")
    print(f"Minimum V_J2 at theta = {float(th_min*180/pi):.1f} deg: {v_min:.6e} m^2/s^2")
    print(f"V_J2 range: {v_max - v_min:.6e} m^2/s^2")
    ...
    print(f"\nAt ISS-like LEO (r = {float(r)/1000:.1f} km, equatorial):")
    print(f"  |a_J2| / |a_monopole| = {ratio_a:.6e}  (~{ratio_a*100:.4f}%)")
    print(f"  J2 perturbation is ~{1/ratio_a:.0f}x smaller than two-body gravity")
