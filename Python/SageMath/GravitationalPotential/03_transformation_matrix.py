...
def main():
    ...
    # Compute R * R^T
    RRT = R * R_transpose
    RRT_simplified = RRT.apply_map(simplify_trig)
    ...
    RTR = R_transpose * R
    RTR_simplified = RTR.apply_map(simplify_trig)
    ...
    det_R = det(R)
    det_R_simplified = simplify_trig(det_R)
    ...
    R_inv = R.inverse()
    R_inv_simplified = R_inv.apply_map(simplify_trig)
    
    print("\nR^(-1) =")
    print(R_inv_simplified)
    
    # Verify R^(-1) = R^T
    diff = (R_inv_simplified - R_transpose).apply_map(simplify_trig)
