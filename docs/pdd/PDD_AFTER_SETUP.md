...
> meaning into one or more versioned PDD `.prompt` files, then regenerates and
> verifies the affected software. User stories and tests independently check
> that the prompts and software still deliver the intended outcome.
...
This is the current, consolidated explanation of the product or larger change.
A friendly conventional name is:
...
useful, especially for a new product or a large change, but it is not the file
PDD compiles for each software component.
...
It is the operative source specification for one manageable part of the
product. It should state that part's purpose, interface, dependencies,
requirements, constraints, examples, outputs, and important `MUST NOT`
behavior.
...
ordinary human intent messages
              |
              v
Product Intent (whole-product, human-readable current truth)
              |
              v
...
2. **Preserve it.** Update the repository's current Product Intent and, when
   useful, preserve the original request in a dated request record.
...
The human may inspect every `.prompt` file, but need not author it manually.
The AI agent is the editor; PDD is the build and verification layer; the human
owns product intent and consequential decisions.

## Agent rule to copy into a repository

```text
For every PDD-managed part, treat ordinary user messages as intent intake, not
as durable source by themselves. Maintain a current human-readable Product
Intent document and versioned PDD .prompt files. Propagate each accepted
behavior change into the affected .prompt before completing implementation.
Use user stories and tests as independent acceptance checks; do not substitute
stories or a PRD for the component prompt. Regenerate and verify only the
affected parts, and report meaning-level prompt changes plus test evidence.
Do not require the user to write formal prompts, stories, PDD commands, or
